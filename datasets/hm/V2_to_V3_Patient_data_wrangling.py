...
# Load csv data into dataframe
df_V2 = pd.read_csv(
    "datasets/hm/raw_data/01_基线.csv",
    delimiter=";",
...
for dict_keys, dict_values in col_names_eq_dict.items():
    df_V2_columns_wrangled.append(
        dict_keys
    )  # V2 CDSL_01 column names dictionary re-ordered

df_V2 = df_V2.reindex(
    columns=df_V2_columns_wrangled
)  # V2 CDSL_01 dataframe columns re-ordered
...
col_aux = [
    ...
    "F_UCI_IN",
    "F_UCI_OUT",
    "HORA_CONSTANTES_PRIMERA_URG",
    "HORA_CONSTANTES_ULTIMA_URG",
]
for col in col_aux:
...
## Dataframe to CSV
df_V2.to_csv("datasets/hm/raw_data/hm_demo.csv", index=False, encoding="utf-8", sep=",")
